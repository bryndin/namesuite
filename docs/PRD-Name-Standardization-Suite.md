...
The main suite operates as a standard Gramps **Tool Addon**, accessible via `Tools -> Family Tree Processing -> Standardize Names...`.

The primary interface utilizes a tabbed layout containing two distinct workspaces. The addon pack also includes a companion Gramplet for inline use.

### 3.1 Tab 1: Rename Given Names

[[RFC - First name standardization]]

Focused on deterministic user-defined rules mapping and typo correction for primary names across any language.
...
### 3.2 Tab 2: Audit Patronymics

Focused on linting existing data for temporal anachronisms, gender mismatches, and lineage errors, as well as inferring and generating missing patronymic names based on lineage data.

* **Configuration Header:**
* `Scope Filter` Dropdown.
* `Configure Rules` Button (Opens a dialog to selectively toggle specific linguistic rules).
* `[] Match Pre-Revolutionary Orthography` (Default: False). Use old orthography instead of modern. (Experimental feature)

* **Action:** `[Audit Database]` Button.

### 3.3 Contextual Sidebar Gramplet
...
3. **Interactive Results Grid:** The UI populates a grid with the findings. The columns adapt based on the active tab:
   * *Rename Tab:* `Use (Checkbox)` | `Gramps ID` | `Person Name` | `Current Name` | `Proposed Name` | `Save name as alternative or not`
   * *Audit Tab:* `Use (Checkbox)` | `Gramps ID` | `Person Name` | `Father's Name` | `Current Patronymic` | `Proposed Fix` | `Confidence` | `Guessed Year` | `Triggered Rule` | `Fix Explanation`
...
5. **Batch Execution:** The user clicks the global `[Apply Selected Corrections]` button at the bottom of the window.
...
* **AC3:** The Audit tab successfully parses standard East Slavic stems and suggests historically accurate suffixes based on lineage data.
