...
def generate_east_slavic_patronymic(
    ...
    epoch = EPOCH_PRE_1917 if (year is not None and year < 1917) else EPOCH_POST_1917

    # Apply Epoch Strategies
    result = ""

    # 1. Pre-1917: Direct Class-Agnostic Genitive Suffix (e.g. Иван Сергеев Коваль)
    if epoch == EPOCH_PRE_1917:
        result = genitive_base if is_male else genitive_base + "а"

    # 2. Post-1917: Modern Formal Gender Suffix (-ovich / -ovna)
    else:
